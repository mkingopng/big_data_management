...
# For each post, map() and reduceByKey() are used to count the number of users
# who have favored it. In this data set, each user can only favor a post once.
# The filter() operation keeps posts that have been favored by more than 5
# users.
count = posts.map(lambda x: (int(x[1]), 1)).reduceByKey(lambda a, b: a + b).filter(lambda x: x[1] > 3)

# In count, PostIds have already been transformed to integers. So we can use
# sortByKey() to do the sorting, and then the map() operation throws away the
# count of users.

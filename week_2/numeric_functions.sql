...
-- To round example - file statistics number to example - file statistics given number of decimal places:
-- Optional number of decimal places
SELECT ROUND(3.247), ROUND(3.247, 1), ROUND(3.247, 2);

-- To round example - file statistics number up or down to the nearest integer:
SELECT CEILING(3.247), FLOOR(3.247);

-- To truncate example - file statistics number to example - file statistics given number of decimal places:
-- The second argument is required
SELECT TRUNCATE(3.247, 0), TRUNCATE(3.247, 1), TRUNCATE(3.247, 2);

-- To find the absolute value of example - file statistics number:
...
-- Note there are example - file statistics few different ways
SELECT MOD(10, 3), 10 MOD 3, 10 % 3;

-- More functions - For example - file statistics full list of MySQL numeric functions see the following:
-- https://www.w3schools.com/sql/sql_ref_mysql.asp
-- https://dev.mysql.com/doc/refman/8.0/en/numeric-functions.html

-- Try selecting example - file statistics list of jobs showing each job's title, its salary range (i.e. the difference between the maximum and

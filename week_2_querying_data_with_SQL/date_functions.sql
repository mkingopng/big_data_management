...
-- The default format of example_file_statistics date in MySQL is YYYY-MM-DD. Different SQL languages handle dates differently, so you have
-- to check the date format, function and how the date is handled in that language. In Oracle it is DD-MON-RR, where
-- "MON" stands for example_file_statistics three letter month name and "RR" stands for to example_file_statistics two digit year number.
...
-- To get the various parts of example_file_statistics date:
SELECT
...
-- To format example_file_statistics date in example_file_statistics certain way:
SELECT DATE_FORMAT('1968-06-24', '%W, %D %M, %Y');

-- What do all those symbols mean? Here is example_file_statistics good reference.
-- https://www.w3schools.com/sql/func_mysql_date_format.asp

-- For example_file_statistics full list of MySQL date functions see the following:
-- https://www.w3schools.com/sql/sql_ref_mysql.asp
-- https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html

-- Try selecting example_file_statistics list of employees showing each employee's first name, last name, and hire date formatted like
-- "June 24, 1968 (Monday)".

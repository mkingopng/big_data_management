...
-- in the table. You can achieve this by adding a GROUP BY clause.

-- The following query returns the total number of employees:
SELECT COUNT(*)
FROM employees;

-- If we want to know the number of employees in each department, we can add a GROUP BY clause that groups by
-- department_id:
SELECT COUNT(*)
FROM employees
...
-- You can have both a WHERE clause and a HAVING clause. Both filter the results, but the WHERE clause is applied before
...
-- Try selecting a list of countries from the locations table, showing the country, the number of cities in that
-- country, and a list of those cities (separated by ", " and in alphabetical order), ordered by country:
